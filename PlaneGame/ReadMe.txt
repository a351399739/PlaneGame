================================================================================
    MICROSOFT 基础类库 : PlaneGame 项目概述
===============================================================================

应用程序向导已为您创建了此 PlaneGame 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。

本文件概要介绍组成 PlaneGame 应用程序的每个文件的内容。
...
PlaneGame.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 CPlaneGameApp 应用程序类。

PlaneGame.cpp
    这是包含应用程序类 CPlaneGameApp 的主应用程序源文件。

PlaneGame.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\PlaneGame.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 PlaneGame.rc 中。
...
PlaneGameDoc.h、PlaneGameDoc.cpp - 文档
    这些文件包含 CPlaneGameDoc 类。编辑这些文件以添加特殊文档数据并实现文件保存和加载（通过 CPlaneGameDoc::Serialize）。
    该文档将包含以下字符串：
        文件扩展名：      data
        文件类型 ID：        PlaneGame.Document
        主框架标题：  PlaneGame
        文档类型名称：       PlaneGame
        筛选器名称：         PlaneGame Files (*.data)
        文件的新短名称：PlaneGame
        文件类型长名称：PlaneGame.Document

PlaneGameView.h、PlaneGameView.cpp - 文档视图
    这些文件包含 CPlaneGameView 类。
	 对象用于查看 CPlaneGameDoc 对象。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 PlaneGame.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
